...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  log("----------------------------------------------------");
  const RandomHexagon = await deploy("RandomHexagon", {
    from: deployer,
    args: args,
    log: true,
  });
  log(`You have deployed an NFT contract to ${RandomHexagon.address}`);
  const networkName = networkConfig[chainId]["name"];
  log(
    `Verify with:\n npx hardhat verify --network ${networkName} ${
      RandomHexagon.address
    } ${args.toString().replace(/,/g, " ")}`
  );
  const RandomHexagonContract = await ethers.getContractFactory(
    "RandomHexagon"
  );
  const accounts = await hre.ethers.getSigners();
  const signer = accounts[0];
  const randomHexagon = new ethers.Contract(
    RandomHexagon.address,
    RandomHexagonContract.interface,
    signer
  );
  ...
  let fund_tx = await linkToken.transfer(RandomHexagon.address, fundAmount);
  ...
  if (chainId != 31337) {
    ...
    const listenForEvent = new Promise(async (resolve, reject) => {
      randomHexagon.once("CreatedUnfinishedRandomSVG", async () => {
        tx = await randomHexagon.finishMint(tokenId, { gasLimit: 2000000 });
        await tx.wait(1);
        log(
          `You can view the tokenURI here ${await randomHexagon.tokenURI(0)}`
        );
        resolve();
      });
    });
    const result = Promise.race([timeout, listenForEvent]);
    tx = await randomHexagon.create({ gasLimit: 300000 });
    let receipt = await tx.wait(1);
    tokenId = (await randomHexagon.tokenCounter()) - 1;
  ...
  log(`You can view the tokenURI here ${await randomHexagon.tokenURI(0)}`);
};

module.exports.tags = ["all", "rhex"];
